...
def process_aruco_marker(marker_id):
    ...
    # Сохраняем данные ArUco-метки в CSV
    save_aruco_data_to_csv(marker_id)


# Функция для записи данных ArUco-метки в CSV файл
def save_aruco_data_to_csv(marker_id, filename="aruco_data.csv"):
    # Открываем файл в режиме добавления
    with open(filename, mode="a", newline="") as file:
        writer = csv.writer(file)
        # Записываем данные и текущее время
        writer.writerow([marker_id, datetime.now().strftime("%Y-%m-%d %H:%M:%S")])
...
def main():
    ...
    while True:
        ...
        cv2.imshow("Socket Camera with QR Code and ArUco Detection", frame)
